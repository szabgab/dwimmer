...
sub add_source {
	...
	$self->dbh->do( "INSERT INTO sources ($fields) VALUES($placeholders)", {}, @$e{@fields} );
...
sub get_queue {
	...
	my $sth = $self->dbh->prepare(
		'SELECT * FROM entries, delivery_queue WHERE entries.id=delivery_queue.entry AND channel = ?');
	$sth->execute($channel);
	my @results;
	while ( my $h = $sth->fetchrow_hashref ) {
		push @results, $h;
...
sub set_config {
	...
	$self->dbh->do( 'INSERT INTO config (key, value, site_id) VALUES (?, ?, ?)',
		undef, $args{key}, $args{value}, $args{site_id} );
	return;
}
