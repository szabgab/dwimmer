...
# adding two sources (feeds)
{
	...
	my ( $out, $err )
		= capture { system "$^X script/dwimmer_feed_admin.pl --store $store --site $site --add < $infile" };
...
# add feed
{
	...
	my ( $out, $err )
		= capture { system "$^X script/dwimmer_feed_admin.pl --store $store --add --site $site2 < $infile" };
...
{
	my ( $out, $err )
		= capture { system "$^X script/dwimmer_feed_admin.pl --store $store --config from foo\@bar.com --site $site" };
	is $out, '', 'no STDOUT Hmm, not good';
	is $err, '', 'no STDERR';
}
{
	my ( $out, $err )
		= capture { system "$^X script/dwimmer_feed_admin.pl --store $store --config another option --site $site" };
	is $out, '', 'no STDOUT Hmm, not good';
	is $err, '', 'no STDERR';
}
...
}
{
	...
	my $data = check_dump($out);
	is_deeply $data, [ [] ], 'config' or diag $out;
	is $err, '', 'no STDERR';
}
{
	my ( $out, $err )
		= capture { system "$^X script/dwimmer_feed_admin.pl --store $store --unconfig another --site $site" };
	is $out, '', 'no STDOUT Hmm, not good';
	is $err, '', 'no STDERR';
}
{
...
{
	my ( $out, $err ) = capture {
		system qq{$^X script/dwimmer_feed_admin.pl --store $store --config html_dir "$html_dir1" --site $site}
	};
	is $out, '', 'no STDOUT Hmm, not good';
	is $err, '', 'no STDERR';
}
...
}
{
	{
		my ( $out, $err )
			= capture { system "$^X script/dwimmer_feed_collector.pl --store $store --collect --verbose" };
...
#like $out, qr{^sources loaded: \d \s* Processing feed $sources[0]{feed} .* Elapsed time: [01]\s*$}x, 'STDOUT is only elapsed time';
	...
	{
		my ( $out, $err ) = capture {
			system qq{$^X script/dwimmer_feed_admin.pl --store $store --config html_dir "$html_dir2" --site $site2}
		};
		is $out, '', 'no STDOUT Hmm, not good';
		is $err, '', 'no STDERR';
	}
